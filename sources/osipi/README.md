...
|---|---|---|
| **Discovery & inventory** | `pi_dataservers`, `pi_points`, `pi_point_attributes` | Establish tag inventory and basic descriptive attributes for selection and governance. |
| **Core time-series** | `pi_streamset_recorded`, `pi_current_value` | Populate primary measurements for analytics and operational monitoring. |
| **Asset Framework (AF)** | `pi_af_hierarchy`, `pi_element_attributes` | Join time-series to assets/hierarchies for rollups and context. |
| **Event Frames** | `pi_event_frames` | Provide time-bounded operational events for batch/downtime/alarm analytics. |
